    ...
    */
    ...
    public function relations()
    {
        ...
        return array(
            ...
            'organizations' => array(self::HAS_MANY, 'Organizations', array('organization__id' => 'organization_id'), 'through'=>'memberships', 'order'=>'name'),
            ...
            'access_grants_experiments' => array(self::HAS_MANY, 'Experiments', array('experiment__id' => 'experiment_id'), 'through'=>'access_grants')
        );
    }
    ...
    public function handleMemberships()
    {
            $organizations = Organizations::model()->findAll();
            foreach($organizations as $organization):
                    $correct_level = $_REQUEST['organization_'.$organization->organization_id.'_level'];
                    $membership = Memberships::model()->find('organization__id=:organization_id and user__id=:user_id',
                            array(':organization_id' => $organization->organization_id, ':user_id' => $this->id));
                    if($correct_level == '' ) {
                            if($membership != null) {
                                    $membership->delete();
                            }
                            ...
                            if($membership != null) {
                                    $membership->level = $correct_level;
                                    $membership->save();
                            } else {
                                    $membership = new Memberships();
                                    $membership->level = $correct_level;
                                    $membership->organization__id = $organization->organization_id;
                                    $membership->user__id = $this->id;
                                    $membership->save();
                            }
                    }
            endforeach;
