	...
	public function actionLogin() {
		...
		if (!Yii::app()->user->isGuest) {
                        ...
                        if($session->get('invite_token') != NULL) {
                                $this->redirect($this->createUrl('/invitations/'.$session->get('invite_token')));
                        } else {
                                $this->redirect($this->createUrl('/experiments'));
                        }
                }
		...
		if (isset($_POST['YumUserLogin'])) {
			...
			if ($success instanceof YumUser) {
				...
				/* 	call_user_func(Yum::module()->afterLogin); */
                                ...
                                $session = Yii::app()->getSession();
                                if($session->get('invite_token') != NULL) {
                                        $this->redirect('/invitations/'<<$session->get('invite_token'));
                                } else {
				        $this->redirect($this->createUrl('/experiments'));
                                }
			} else
