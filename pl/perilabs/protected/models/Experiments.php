...
class Experiments extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('organization__id, user__id', 'numerical', 'integerOnly'=>true),
	...
	public function relations()
	{
		...
		return array(
		    ...
		    'conclusions' => array(self::HAS_MANY, 'Conclusions', 'experiment__id'),
                    ...
                    'users' =>        array(self::BELONGS_TO, 'User', 'user__id'),
		);
	}
	...
	public function search()
	{
        ...
        */
         ...
         */
        public function copyExp($model, $copy, $attributes = array())
        {
                ...
                if($copy->save()) {
                    if(isset($model->variablesTimes) and !empty($model->variablesTimes)) {
                            foreach($model->variablesTimes as $varTime):
                                    $this->copyExp($varTime, $m = new VariablesTime('copy'), array('variable__id' => $copy->variable_id));
                            endforeach;
                    }
                    if(isset($model->variablesTexts) and !empty($model->variablesTexts)) {
                            foreach($model->variablesTexts as $varText):
                                    $this->copyExp($varText, $m = new VariablesText('copy'), array('variable__id' => $copy->variable_id));
                            endforeach;
                    }
                    return true;
                } else {
                    error_log("couldn't copy something");
                    return false;
                }
        }
