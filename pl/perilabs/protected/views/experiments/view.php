...
<div class="row-fluid">
	...
	<ul id="metrics-list">
		...
		?>
                ...
                <!-- CONCLUSIONS -->
	<?php if($model->locked) { ?>
        	<h3 class="conclusions">Conclusions
		<?php if(AccessControl::canGatherExperiment($model->experiment_id) and $model->locked): ?>
        	    <?php echo CHtml::link(
        	        '<b>+</b> Add Conclusions',
        	        $this->createUrl('/conclusions/create?experiment_id='.$model->experiment_id.'#create-conclusion-form'),
        	        array('data-toggle'=>'collapse', 'class'=>'btn btn-small', 'data-remote'=>'true', 'style'=>'line-height: 12px; padding: 3px 5px;')
        	    ); ?>
        	    <a href="#" id="popover" rel="popover" title="Add Conclusions" data-content="Conclusions are important."><img src="/images/help.png" alt="Help popover"/></a>
		<?php endif;?>
		</h3>
        	<?php $this->renderPartial("//conclusions/_ajax_form", array("model"=> new Conclusions(), "experiment_id" => $model->experiment_id)) ?>
        	<ul id="conclusions-list">
        	        <?php foreach ($model->conclusions as $conclusion):
        	                $this->renderPartial("//conclusions/_list_view", array("model" => $conclusion, "experiment_id" => $model->experiment_id));
        	                endforeach;
        	        ?>
        	</ul>
	<?php } ?>
...
<h3 class="margintop">When you are done adding Constants, Variables, and Metrics, lock your experiment and the trials will be generated.</h3>
</br>
<h3>Reference materials for designing experiments:</h3>
<?php echo CHtml::link("Detailed Experiment Introduction", array('site/page', 'view'=>'design')) ?></br>
<?php echo CHtml::link("Experiment Design Reference Chart", array('site/page', 'view'=>'reference')) ?>
