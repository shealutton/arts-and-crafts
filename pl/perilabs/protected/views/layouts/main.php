...
if (Yii::app()->user->isGuest) {
	...
	$login_form = '<form class="pull-right navbar-login" action="'.$this->createUrl('/experiments/search').'" method="post">
				<input name="SearchForm[search_txt]" id="SearchForm_search_txt" type="text" placeholder="search experiments" size="10" style="width: 10em; margin: 5px 0px 0px 0px;" />
				<input name="returnUrl" value="/experiments" type="hidden" />
...
$brand = '<img src="'.Yii::app()->baseUrl.'/images/RedLogoSmall.png" style="width: 167px; height: 40px;" alt="PeriLabs" />';
	 ...
	 'items'=>array(
		...
		'items' => array( //items chunk #1
          ...
          array('label'=>($locked ? 'Gather' : 'Design'), 'url'=>array('/experiments/'.$experiment_id ), 'visible'=>!Yii::app()->user->isGuest && isset($this->experiment) && $this->experiment != null,
          	'active'=>
                (Yii::app()->controller->id=='experiments' and Yii::app()->controller->getAction()->getId()!='analyze') |
          ...
          array('label'=>'Analyze', 
          'url'=>array('/experiments/analyze?experiment_id='.$experiment_id), 
          'itemOptions'=>array('class'=>'disabled'), 
          'linkOptions'=>array('class'=>'disabled'), 
          'active'=>Yii::app()->controller->getAction()->getId() == 'analyze',
          'visible'=>!Yii::app()->user->isGuest && isset($this->experiment) && $this->experiment != null && $locked),
  ...
  <div class="subnav subnav-fixed">
    ...
    <div class="floatRight copyright-notice">
      Copyright &copy; <?php echo date('Y'); ?> by Peri Labs, Inc. All Rights Reserved.
    </div>
</div>
